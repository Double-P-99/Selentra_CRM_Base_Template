...
from crm_core.api.schemas import (
    ActivityIn,
    ActivityOut,
    CompanyIn,
    CompanyOut,
...
from crm_core.selectors.opportunity_selectors import get_opportunity_by_id
from crm_core.selectors.opportunity_selectors import list_opportunities as list_opportunities_selector
from crm_core.selectors.reports_selectors import (
...
def create_company(request, data: CompanyIn):
    return Company.objects.create(**data.dict())
...
def create_contact(request, data: ContactIn):
    return Contact.objects.create(**data.dict())
...
def create_pipeline(request, data: PipelineIn):
    return Pipeline.objects.create(**data.dict())
...
def move_stage(request, opportunity_id: int, data: OpportunityMoveStageIn):
    ...
    return OpportunityService.move_stage(
        opportunity=opportunity, to_stage=to_stage, user=request.auth, notes=data.notes
    )
...
def create_activity(request, data: ActivityIn):
    return Activity.objects.create(**data.dict())
...
def create_visit(request, data: VisitIn):
    return Visit.objects.create(**data.dict())
